...
def execute_sql_file(connection, sql_file: Path):
    ...
    try:
        ...
        try:
            ...
            if error_count == 0:
                print(f"\n[SUCCESS] 成功执行 {success_count} 条 SQL 语句")
                return True
            else:
                print(f"\n[WARN] 成功 {success_count} 条，失败 {error_count} 条")
                return False
...
def main():
    ...
    # 获取迁移脚本
    migration_file = Path(__file__).parent.parent / "migrations" / "2025120901_add_user_id_to_entity_types.sql"
    ...
    if execute_sql_file(connection, migration_file):
        ...
        try:
            cursor = connection.cursor()
            cursor.execute("DESCRIBE entity_types")
            columns = cursor.fetchall()
            user_id_exists = any(col['Field'] == 'user_id' for col in columns)
            
            if user_id_exists:
                print(f"\n[VERIFY] 验证成功: user_id 字段已添加到 entity_types 表")
            else:
                print(f"\n[WARN] 验证失败: user_id 字段未找到")
