...
# 读取迁移脚本
migration_file = project_root / 'migrations' / '2026011902_code_qa_storage.sql'
...
try:
    ...
    for i, statement in enumerate(statements, 1):
        ...
        try:
            ...
            # 检查是否是表已存在的错误
            if error_code == 1050 and 'already exists' in error_msg:
                print(f"  [WARNING] 表已存在，跳过: {error_msg}\n")
            elif error_code == 1060 and 'Duplicate column name' in error_msg:
    ...
    print("=" * 60)
    print("\n迁移内容:")
    print("  - 创建 code_qa_sessions 表（代码问答会话表）")
    print("  - 创建 code_qa_records 表（代码问答记录表）")
    print("\n现在系统可以正常保存代码问答记录了！")
